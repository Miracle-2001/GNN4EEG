...
class Trainer(object):
    def __init__(self, model_f, num_nodes, num_hiddens=400, num_classes=2,
        ...
        self.model_name = model_f.__name__
        self.early_stop = early_stop
        if extension is not None:
            self.__dict__.update(extension)
        self.trainer_config_para = self.__dict__.copy()  # save the training configs exclude model,num_features and model_config_para
        self.model = None
                ...
                with torch.no_grad():
                    for name,para in self.model.named_parameters():
                        if name in ['edge_weight']:
                            tmp=torch.sign(para.data)*torch.maximum(torch.zeros_like(para.data),torch.abs(para.data)-0.5*self.lr*self.l1_reg)
                            para.copy_(tmp)
    ...
    def train_and_eval(self, train_data, train_label, valid_data, valid_label, fold, model_path, model_name,
        ...
        # draw acc&loss fig
        draw(last_epoch, train_acc_list, eval_acc_list,
             train_loss_list, eval_loss_list, model_path, model_name)
        ...
        def default_dump(obj):
            ...
            if isinstance(obj, (np.integer, np.floating, np.bool_)):
                ...
                return obj
        # save acc and loss into a json file
        json.dump({'fold': int(fold),
                   'train_acc_list': train_acc_list,
                   'eval_acc_list': eval_acc_list,
                   'train_loss_list': train_loss_list,
                   'eval_loss_list': eval_loss_list,
                   'train_num_correct_list':train_num_correct_list,
                   'eval_num_correct_list':eval_num_correct_list,
                   'best_acc': best_acc,
                   'best_epoch': best_epoch
                   }, open(model_path +"/"+
                           model_name.split(".")[0]+ '_acc_and_loss.json', 'w'),default=default_dump)
        # reload the best parameters
        if reload:
            self.load(model_path, model_name)

        print('fold ', fold, 'best acc ', best_acc,
              ' best epoch ', best_epoch)
        
        if nd_predict:
            preds_train=self.do_epoch(train_loader, 'eval', -1,ret_predict=True)
            preds_val=self.do_epoch(valid_loader, 'eval', -1,ret_predict=True)
