...
def benchmark_new(args):
    ...
    for fold in fold_list:
        ...
        if args.model == 'SVM':
            ...
            for C in rangeDict["C"]:
                start_time0 = time.time()
                newargs.C = C
                now_para_dir = os.path.join(now_fold_dir, f'C={C}')
                os.makedirs(now_para_dir)
                mean_acc = {"val": 0, "train": 0}
                for sub_fold in range(3):
                    ...
                    data_train = data_train.reshape(-1, 30*args.band)
                    data_val = data_val.reshape(-1, 30*args.band)
                    trainer = get_trainer(newargs)

                    start_time = time.time()
                    trainer.fit(data_train, label_train)

                    # joblib.dump(trainer, os.path.join(
                    #     now_para_dir, model_name))
                    preds_train = trainer.predict(data_train)
                    preds_val = trainer.predict(data_val)
                    end_time = time.time()
                    # print("numofpreds_train ",preds_train.shape)
                    train_num_correct = np.sum(
                        preds_train == label_train)
                    train_acc = train_num_correct / len(label_train)
                    val_num_correct = np.sum(preds_val == label_val)
                    val_acc = val_num_correct / len(label_val)
                    print('thread id:', args.thread_id, ' fold:', fold, 'C:', C, 'sub_fold:', sub_fold, 'train acc:', train_acc,
                          'val acc:', val_acc, 'time consumed:', end_time - start_time)
                    json.dump({'fold': int(fold),
                               ...
                               'time consumed': end_time - start_time
                                       ...
                                       f'/fold_{fold}_subfold_{sub_fold}_acc_and_loss.json', 'w'))
                    mean_acc["val"] += val_num_correct
                    mean_acc["train"] += train_num_correct

                mean_acc["val"] /= num_train_and_val
                mean_acc["train"] /= 2*num_train_and_val
                para_result_dict.update({f"C={C}": mean_acc["val"]})
                json.dump({'fold': int(fold),
                           'train_acc': mean_acc["train"],
                           'val_acc': mean_acc["val"],
                           'C': C,
                           'time consumed': end_time - start_time
                           }, open(now_para_dir +
                                   f'/fold_{fold}_mean_acc_and_loss.json', 'w'))

                if mean_acc["val"] > best_acc["val"]:
                    best_acc["val"] = mean_acc["val"]
                    best_acc["train"] = mean_acc["train"]
                    best_C = C
            print('best_C: ', best_C, ' best_acc val: ',
                  best_acc['val'], ' best_acc train: ', best_acc['train'])
            best_para_dict.update({'C': best_C})
            newargs.C = best_C
            trainer = get_trainer(newargs)
            start_time = time.time()
            # all of the train_and_val
            data_train_and_val = data_train_and_val.reshape(-1, 30*args.band)
            data_test = data_test.reshape(-1, 30*args.band)
            trainer.fit(data_train_and_val, label_train_and_val)
            ...
            best_acc = {"val": 0, "train": 0}
            count = 0
            for lr in rangeDict["lr"]:
                for num_hiddens in rangeDict["num_hiddens"]:
                    for l1_reg, l2_reg in zip(rangeDict["l1_reg"], rangeDict["l2_reg"]):
                        ...
                        for sub_fold in range(3):
                            ...
                            trainer.train(data_train, label_train, data_val,
                                        label_val, sub_fold, now_para_dir, model_name,reload=False)
            ...
            model_name = 'fold_%s_model.joblib' % (
                            str(fold))
            trainer.train(data_train_and_val, label_train_and_val, data_test,
